...
def main():
    input_file = #path to test .csv file
    ...
    predictions_df['time_delay'] = 0.45385 - 0.44863 * np.log10(predictions_df['Ix'])
    z=74 #Adjust the z value, default is 74 km
    predictions_df['Ne'] = 1.43 * 10**13 * np.exp(-0.15 * predictions_df['Predicted_Reflection_height']) * np.exp((predictions_df['Predicted_Beta'] - 0.15) * (z - predictions_df['Predicted_Reflection_height']))
